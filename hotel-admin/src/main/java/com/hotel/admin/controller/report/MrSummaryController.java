package com.hotel.admin.controller.report;

import com.hotel.admin.model.MrSummary;
import com.hotel.admin.model.WrSummary;
import com.hotel.admin.service.MrSummaryService;
import com.hotel.admin.service.WrSummaryService;
import com.hotel.core.http.HttpResult;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

@RestController
@RequestMapping("mrSummary")
public class MrSummaryController {

    @Autowired
    private MrSummaryService mrSummaryService;

    @PostMapping("/page")
    public HttpResult findPage(MrSummary mrSummary) {
        return HttpResult.ok(mrSummaryService.findPage(mrSummary));
    }

}
